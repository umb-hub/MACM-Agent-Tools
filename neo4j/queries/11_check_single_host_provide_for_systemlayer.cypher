// Reporting query for single host/provide for SystemLayer
MATCH (s:SystemLayer)
OPTIONAL MATCH (hoster)-[r]->(s)
WHERE type(r) IN ["hosts","provides"]
WITH s, COUNT(DISTINCT hoster) AS numHosts
WHERE numHosts <> 1
WITH s, numHosts,
     CASE 
        WHEN s.component_id IS NOT NULL AND s.name IS NOT NULL 
        THEN "SystemLayer[component_id: " + s.component_id + ", name: " + s.name + "] has " + toString(numHosts) + " host(s)"
        WHEN s.component_id IS NOT NULL 
        THEN "SystemLayer[component_id: " + s.component_id + "] has " + toString(numHosts) + " host(s)"
        WHEN s.name IS NOT NULL 
        THEN "SystemLayer[name: " + s.name + "] has " + toString(numHosts) + " host(s)"
        ELSE "SystemLayer[id: " + toString(id(s)) + "] has " + toString(numHosts) + " host(s)"
     END AS violationDetail
RETURN violationDetail;
