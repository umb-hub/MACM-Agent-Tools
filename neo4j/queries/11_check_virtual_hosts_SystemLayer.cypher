...
RETURN apoc.text.format("Invalid Virtual-to-SystemLayer hosting relationship: %s -[:hosts]-> %s\n  Source type: %s\n  Target type: %s\n  Rule: Virtual nodes can only host SystemLayer.OS and SystemLayer.Firmware (base system layers)", [sourceDescription, targetDescription, coalesce(sourceNode.type, "<no-type>"), coalesce(targetNode.type, "<no-type>")]) AS violationDetail;
