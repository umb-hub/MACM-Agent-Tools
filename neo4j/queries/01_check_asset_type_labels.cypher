// Reporting query converted from trigger 01_check_asset_type_labels
...
WITH n, plFromType, lbls, 
     CASE WHEN NOT (plFromType IN lbls) 
          THEN errors + [apoc.text.format("/* %s type=%s expects primary label %s but labels are [%s] */", [coalesce(n.name,"<no name>"), n.type, plFromType, apoc.text.join(lbls, ",")])]
          ELSE errors 
     END AS errors
...
WITH n, plFromType, lbls, rest, expectedSome, noneAllowed, 
     CASE WHEN hasAnyMapping = 0 
          THEN errors + [apoc.text.format("/* %s type=%s PL=%s is not covered by mapping */", [coalesce(n.name,"<no name>"), n.type, plFromType])]
          ELSE errors 
     END AS errors
...
WITH n, plFromType, lbls, rest, expectedSome, noneAllowed, expectedStr, restStr,
     CASE WHEN (noneAllowed AND size(rest) <> 0) OR (NOT noneAllowed AND (size(rest) <> 1 OR NOT rest[0] IN expectedSome))
          THEN errors + [apoc.text.format("/* %s type=%s PL=%s expected SL(s)=%s but secondary labels are [%s] */", [coalesce(n.name,"<no name>"), n.type, plFromType, expectedStr, restStr])]
          ELSE errors 
     END AS errors
...
UNWIND allNodeReports AS report
RETURN report;
